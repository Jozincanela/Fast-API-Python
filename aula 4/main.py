...
@app.get("/covid/mortes_desc", response_class=HTMLResponse)
def read_deaths_desc(request: Request):
    url = 'https://covid19-brazil-api.vercel.app/api/report/v1'
    dados_covid = get_data_url(url)
    organized_deaths_desc = organize_deaths_list(dados_covid)

    return templates.TemplateResponse(
        'desc_mortes.html',
        {"request": request, 'organized_deaths_desc': organized_deaths_desc }
    )
    
    

@app.get("/covid/mortes_cresc", response_class=HTMLResponse)
def read_deaths_cresc(request: Request):
    url = 'https://covid19-brazil-api.vercel.app/api/report/v1'
    dados_covid = get_data_url(url)
    organized_deaths_cresc = organize_deaths_list_reverse(dados_covid)

    return templates.TemplateResponse(
        'cresc_mortes.html',
        {"request": request, 'organized_deaths_cresc': organized_deaths_cresc }
    )